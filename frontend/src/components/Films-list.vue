<template>
  <div class="mx-auto flex justify-center items-center p-2 md:p-0 mt-6">
    <div
      class="grid gap-4"
      :class="showedArrows ? 'grid-cols-5' : 'grid-cols-3'"
    >
      <films-list-arrows
        :show-arrows="showedArrows"
        @prev="prevArrowClicked"
        @next="nextArrowClicked"
      >
        <template v-slot:prev-arrow />

        <template v-slot:default>
          <film-item
            v-for="movie in showedMovies"
            :title="movie.title"
            :year="movie.year"
            :genres="movie.genres"
